...
log_dir = './log'
load_model = './model_save/mbart-large-cc25/model_epoch_0_20230716055827.pt'
...
def check(filename_log, origin, generated, truth):
    with open(filename_log, 'a') as f:
...
class MyModel(nn.Module):
    ...
    def generate(self, input_ids, labels, decoder_start_token):
        ...
        truth_sentences = self.tokenizer.batch_decode(input_ids, skip_special_tokens=True)[0]
        return truth_sentences, generated_sentences, ground_truth_sentences
...
def train_eval(model, optimizer, train_loader, val_loader, test_loader, epochs=50):
    ...
    for epoch in range(epochs):
        ...
        for batch_idx, batch in enumerate(tqdm(train_loader)): 
            if batch_idx % 500 == 1:
                truth_sentences, generated_sentences, ground_truth_sentences = model.generate(batch[0], batch[1], 'zh_CN')             
            ...
            if batch_idx % 500 == 1:
                ...
                check(log_dir, truth_sentences, generated_sentences, ground_truth_sentences)
    ...
    for batch_idx, batch in enumerate(tqdm(val_loader)):
        ...
        with torch.no_grad():
            ...
            if batch_idx %500 == 1:
                ...
                write_log_valid(log_dir, batch_idx,epoch, eval_loss/batch_idx)
                check(log_dir, truth_sentences, generated_sentences, ground_truth_sentences)
