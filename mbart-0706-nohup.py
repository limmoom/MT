...
def write_log_train(filename_log, epoch, loss):
    with open(filename_log, 'a') as f:
        f.write(f'epoch: {epoch}, train_loss: {loss}\n')

def write_log_valid(filename_log, epoch, loss):
    with open(filename_log, 'a') as f:
        f.write(f'epoch: {epoch}, valid_loss: {loss}\n')
...
def train_eval(model, optimizer, train_loader, val_loader, test_loader, epochs=50):
    ...
    for epoch in range(epochs):
        ...
        for batch_idx, batch in enumerate(tqdm(train_loader)): 
            ...
            optimizer.step()
            if batch_idx % 500 == 1:
                print(f"Epoch [{epoch + 1}/{epochs} : batch_idx{batch_idx}] - Train Loss: {train_loss/batch_idx:.4f}")
                write_log_train(log_dir, epoch, train_loss)
    ...
    for batch_idx, batch in enumerate(tqdm(val_loader)):
        ...
        with torch.no_grad():
            ...
            if batch_idx %500 == 1:
                ...
                write_log_valid(log_dir, epoch, eval_loss)
